...
- `go.mod` is `go 1.25.0` with three direct dependencies: `github.com/goccy/go-yaml`, `github.com/modelcontextprotocol/go-sdk` (pinned to the v1.7 minor) and `golang.org/x/sys` (ADR 0011; imported by `internal/audit` on Windows for the key-file DACL). Never add `gopkg.in/yaml.v3` (unmaintained).
