...
# Permissive licences whose modules may be linked into the binary, which is
# distributed under FSL-1.1-ALv2 (ADR 0034) and converts to Apache-2.0: every
# licence here is compatible with both. A module under anything else fails the
# run until someone decides (and records) what to do about it.
...
def render(dest: pathlib.Path) -> list[str]:
    ...
    index = [
        ...
        "Fathomgate itself is under the Functional Source License 1.1, ALv2 Future License (`FSL-1.1-ALv2`) in `LICENSE`, except `policies/examples/` and `profiles/` (Apache-2.0); attributions are in `NOTICE`. "
