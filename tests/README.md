...
- Tier 2: live for netdev-ssh-mcp v1.7.1 (CI job `client-smoke`, no Docker).
  ...
  plus the upstream's own keyed `[h:...]` tokens (with a FAKE key file and
  with its per-run random key) and its refusal of injected commands, the
  fixes for GHSA-8g43-jrf3-q9vq and GHSA-h47r-329w-6p9h;
