...
#
# A leg is a chain (with or without netguard) and an upstream fixture,
# chosen separately below: how the suite reaches the chain (the relay today,
# netguard's HTTP listener in T0.32) does not touch upstream selection, and
# a new upstream does not touch the transport.
#
#   leg              chain                                upstream
#   control          relay -> upstream                    current
#   netguard         relay -> netguard serve -> upstream  current
#   control-up2025   relay -> upstream                    2025
#   netguard-up2025  relay -> netguard serve -> upstream  2025
#
#   upstream  current  go-sdk conformance/everything-server at the go.mod
#                      version; negotiates 2026-07-28 with netguard
...
# only on the matching netguard leg belongs to netguard. control-up2025 has
...
usage() {
  echo "usage: $0 <control|netguard|control-up2025|netguard-up2025> <2025-11-25|2026-07-28>" >&2
  exit 2
}
[ $# -eq 2 ] || usage
leg=$1
...
case "$leg" in
  ...
  netguard) chain=netguard upstream=current ;;
  control-up2025) chain=direct upstream=2025 ;;
  netguard-up2025) chain=netguard upstream=2025 ;;
  *) usage ;;
esac
if [ "$chain" = direct ] && [ "$upstream" = 2025 ] && [ "$rev" = 2026-07-28 ]; then
  echo "== conformance $leg $rev: skipped (a 2025-only upstream has no 2026-07-28 run without netguard in front)"
  exit 0
fi
