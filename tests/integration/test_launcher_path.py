...
beyond /usr/bin:/bin, sometimes none. fathomgate resolves a bare `--upstream`
...
The device password reaches fathomgate the way a client's `env` block puts it
...
def _assert_fails_fast(client: RawClient) -> str:
    ...
    assert "fathomgate: proxy: upstream netdev-ssh-mcp: connect:" in err, err
    return err


@pytest.mark.asyncio
async def test_empty_path_absolute_paths_serve_tools_list(fathomgate_binary: Path, upstream_binary: Path, fake_device: FakeDevice) -> None:
    ...
    params = StdioServerParameters(command=str(fathomgate_binary), args=serve_args(upstream_binary, fake_device), env=client_env({"PATH": ""}))
...
def test_env_i_absolute_paths_serve_tools_list(fathomgate_binary: Path, upstream_binary: Path, fake_device: FakeDevice) -> None:
    ...
    `--upstream-env`; fathomgate itself needs nothing from the environment
    beyond the password the client's `env` block sets."""
    client = RawClient([str(fathomgate_binary), *serve_args(upstream_binary, fake_device)], env=client_env())
    try:
...
def test_env_i_without_home_fails_fast_with_upstream_reason(fathomgate_binary: Path, upstream_binary: Path) -> None:
    """Same, without SSH_KNOWN_HOSTS: the upstream exits at startup and its
    reason reaches stderr, labelled with the server name."""
    client = RawClient([str(fathomgate_binary), *serve_args(upstream_binary, None)], env=client_env())
...
def test_bare_upstream_name_fails_fast(fathomgate_binary: Path, upstream_binary: Path, path: str) -> None:
    ...
    client = RawClient([str(fathomgate_binary), *serve_args(SERVER, None)], env=client_env(env))
    err = _assert_fails_fast(client)
...
def test_path_lookup_launcher_fails_fast_without_path(fathomgate_binary: Path, upstream_binary: Path, fake_device: FakeDevice, tmp_path: Path) -> None:
    client = RawClient([str(fathomgate_binary), *serve_args(_launcher(tmp_path, upstream_binary), fake_device)], env=client_env({"PATH": "/usr/bin:/bin"}))
    err = _assert_fails_fast(client)
    assert "upstream netdev-ssh-mcp:" in err and "not found" in err


def test_path_lookup_launcher_works_with_upstream_env_path(fathomgate_binary: Path, upstream_binary: Path, fake_device: FakeDevice, tmp_path: Path) -> None:
    ...
    argv = [str(fathomgate_binary), *serve_args(_launcher(tmp_path, upstream_binary), fake_device, "--upstream-env", f"PATH={path}")]
...
def test_password_is_never_on_the_command_line(upstream_binary: Path, fake_device: FakeDevice) -> None:
    ...
    the fake device with it) without ever being in fathomgate's argv."""
...
def test_unset_password_exits_2_naming_the_variable(fathomgate_binary: Path, upstream_binary: Path, fake_device: FakeDevice) -> None:
    """A client without the `env` block: fathomgate refuses to start (exit 2,
    before anything is spawned) and names the variable, never a value."""
    client = RawClient([str(fathomgate_binary), *serve_args(upstream_binary, fake_device)], env={})
    code, err, _ = client.wait(timeout=FAIL_FAST_SECONDS)
    assert code == 2, (code, err)
    assert "--upstream-env-pass DEVICE_PASSWORD: not set in fathomgate's environment" in err, err
