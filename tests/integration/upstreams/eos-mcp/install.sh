...
#            its dependencies, every wheel hash-checked (--require-hashes).
#            The tests also check each installed eos_mcp/*.py against the
#            sha256 of the same file at commit bffb893 (the tag v1.3.0, which
#            profiles/eos-mcp.yaml was read at).
#
...
# requirements.in/.txt and in tests/integration/conftest.py together.
set -eu
...
VIRTUAL_ENV="$dest/venv" uv pip install -q --require-hashes -r "$here/requirements.txt"
