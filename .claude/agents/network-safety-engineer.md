...
6. For inventory: `fathomgate inventory import tests/fixtures/inventory/devices.csv`, `fathomgate inventory lint` on the result (no problems; review any pattern warnings), then `fathomgate inventory resolve core-rtr-01` must print `{role: core, site: ..., tags: [...], source: static, stale: false}`; with a test resolver at order 4 that is unreachable and serves a snapshot, confirm the same resolve prints `source: snapshot, stale: true` and a `WRITE_CONFIG` is still `hold`, not `allow`.
