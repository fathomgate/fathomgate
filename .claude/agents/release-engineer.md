...
- Never tag a public release under a name whose naming ADR is not `accepted`. Until then, snapshot builds only (`goreleaser release --snapshot --clean --skip=sign`).
...
2. Preflight on `main` at the candidate commit: `git status --porcelain` empty; `go test ./... -race && golangci-lint run && make policy-test && make conformance`; the tier 2 suite from `tests/`, `uv run --extra integration pytest integration -m "tier2 and <server>"` for each of `netdev_ssh_mcp`, `upa_mcp_netmiko_server` and `eos_mcp` (the `tier2` CI jobs on the candidate commit are the same runs); the `gitleaks` CI job is green on the candidate commit (a push to `main` scans the full history with the pinned gitleaks, `.gitleaks.toml` and `.gitleaksignore`; M1-42) and `make secrets-control && make secrets-scan` passes locally (a working-tree `gitleaks dir` or `--no-git` scan re-reports the false positives `.gitleaksignore` lists by commit, so it is not the gate); `go mod verify`; `go version -m ./fathomgate | grep go-sdk` shows the pinned minor.
3. Snapshot build and inspect: `goreleaser release --snapshot --clean --skip=sign` (keyless cosign signing needs the CI release job's OIDC identity); for each archive `file dist/fathomgate_*/fathomgate*` says statically linked; `dist/fathomgate_linux_amd64_v1/fathomgate version` prints the expected version; `syft dist/… -o spdx-json` present; `dist/checksums.txt` lists every archive and SBOM (the `cosign sign-blob` signature is made and checked only in CI and post-release, not locally).
...
5. Snippets: run the tier-2 PATH-stripped launcher case against each of the three `mcp.json` snippets in `docs/install.md` (from `tests/`, `uv run --extra integration pytest integration/test_launcher_path.py -v` (matrix row 22, marked `tier2 and netdev_ssh_mcp`, so the `tier2 client smoke (netdev-ssh-mcp)` CI job runs it too)). Update paths if the binary location or flags changed.
...
7. Tag: `git tag -s vX.Y.Z -F <(sed -n '/^## \[X.Y.Z\]/,/^## \[/p' CHANGELOG.md)` and `git push origin vX.Y.Z`. Watch `.github/workflows/release.yaml`: when the `goreleaser` job waits for the `release` environment, check that the run is for this tag and that the `test` job passed, then approve the deployment. After it finishes: `brew install fathomgate/tap/fathomgate && fathomgate version`; `cosign verify-blob --certificate … --signature … dist/checksums.txt`; `docker pull ghcr.io/fathomgate/fathomgate:vX.Y.Z` and `cosign verify`.
