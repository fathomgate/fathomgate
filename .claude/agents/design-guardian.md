...
5. CLI order check: run `netguard policy eval --policy policies/examples/read-only.yaml --tool netdev-ssh-mcp.run_show_command --arg host=lab-sw-01 --arg command=reload` and confirm the line reads decision, class, target, rule, reason. Compare with the console meta order in `design/preview.html`.
