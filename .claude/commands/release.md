...
2. Preflight on the candidate commit on `main`: `git status --porcelain` is empty; `go test ./... -race -count=1 && golangci-lint run && make policy-test && make conformance`; `uv run pytest tests/tier2 -q`; `gitleaks detect --no-git --source .`; `go mod verify`; `go version -m ./fathomgate | grep modelcontextprotocol/go-sdk` shows the pinned minor.
