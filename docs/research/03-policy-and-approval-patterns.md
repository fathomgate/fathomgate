...
- **Update 2026-09-23 [F, src] (T0.29).** At v1.6.6 (`be3e363`) the hash is plain `sha256(value)` cut to 12 hex characters, with no key, so short values are recoverable from a word list. Several of its regexes also hash a keyword (`sha512`, `7`, `ascii-text`) and leave the real secret in clear. The source reading and a run against NetGuard's fixtures are in `02-network-mcp-servers.md` §1.1, "Update 2026-09-23". This confirms the design note below: NetGuard uses keyed HMAC (invariant 4) and does not rely on the upstream.
