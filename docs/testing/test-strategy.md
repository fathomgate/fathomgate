...
- Passes means every scored check passes except those in `tests/conformance/baseline/`, each listed per check with its reason: capabilities netguard does not declare, input requests it refuses, log messages it does not relay, answers it will not forward without its `requestState`, the era pairing and tool prefix, and HTTP-transport checks that belong to the relay. Each names the ADR or board task behind it. A baseline entry that starts passing fails the run.
