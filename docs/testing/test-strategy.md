...
- Not covered: a FastMCP upstream (the 2025 fixture is go-sdk's server held on an older release, not a different SDK), and the listener's session lifecycle after the fourth leaked session: the suite does not DELETE the session of a scenario that throws, so on the 2025-11-25 fathomgate legs the per-principal cap of 4 sessions answers later scenarios with 503 (tests/conformance/README.md, "One process"). Tier 1 covers the lifecycle.
