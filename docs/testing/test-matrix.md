...
| 2 | Dual-era handshake | 2 | netdev-ssh-mcp (go-sdk, 2026 era); upa/mcp-netmiko-server (FastMCP, 2025 era) | Both upstreams initialise; conformance suite green on the client-facing side | M0 | passing (tier 2 CI on `main` at `51921ed`, 2026-09-24): netdev-ssh-mcp v1.6.6 initialises behind fathomgate at 2026-07-28; upa/mcp-netmiko-server `96e8ff3` initialises at 2025-11-25 with mcp 1.30.0, for a 2025-11-25 agent and a 2026-07-28 agent, and at 2024-11-05 on its own `uv.lock` (mcp 1.6.0) after fathomgate restarts it once and connects with `initialize` only ([ADR 0018](../adr/0018-bound-server-discover-then-initialize-only.md)); `mcp-conformance` green. Since T0.51 the netdev-ssh-mcp half runs v1.7.1, still at 2026-07-28. See [run notes](#run-notes) |
...
| 22 | PATH-stripped launcher | 2 | Claude Desktop-style config with absolute binary path and empty `PATH`; netdev-ssh-mcp v1.7.1 behind fathomgate (v1.6.6 until T0.51) | Proxy starts and serves `tools/list`; no ENOENT | M0 | passing (client-smoke CI, netdev-ssh-mcp v1.7.1 since T0.51; Claude Code 2.1.236 and Claude Desktop 2.7032.0 by hand with v1.6.6, 2026-09-23; see [run notes](#run-notes)) |
...
**Rows 1, 2, 15 and 22, 2026-09-25 (T0.51): netdev-ssh-mcp v1.6.6 to v1.7.1.** Upstream: krisiasty/netdev-ssh-mcp v1.7.1, tag commit `6fc6ab0aed1e84b8dd71b1bf1ad39cd909146623`. It fixes [GHSA-8g43-jrf3-q9vq](https://github.com/krisiasty/netdev-ssh-mcp/security/advisories/GHSA-8g43-jrf3-q9vq) (v1.7.0: keyed obfuscation tokens, the T0.29 report) and [GHSA-h47r-329w-6p9h](https://github.com/krisiasty/netdev-ssh-mcp/security/advisories/GHSA-h47r-329w-6p9h) (v1.7.1: command injection). The tool names, parameters and go-sdk v1.8.0 are unchanged; the source reading is in `profiles/netdev-ssh-mcp.yaml`. [PR #108](https://github.com/fathomgate/fathomgate/pull/108), fathomgate at `3a49077`.

- CI: job `tier2 client smoke (netdev-ssh-mcp)` ([job 107922178797](https://github.com/fathomgate/fathomgate/actions/runs/36087378002/job/107922178797)), linux_amd64 release binary (sha256 `f90795b4…0c9e7`, from the release's `checksums.txt`, `sha256sum -c` OK). `-m "tier2 and netdev_ssh_mcp"`: 23 passed, 2 skipped (M1), 1 xfailed (row 15). Every other job in [run 36087378002](https://github.com/fathomgate/fathomgate/actions/runs/36087378002) is green.
...
- Row 1: unchanged. The five prefixed tools, `show version` once on the device, and the upstream's refusal of `reload` as `isError` with nothing sent. New in `test_passthrough.py`:
  ...
  - With no key: four distinct `[h:…]` tokens, none of them the v1.6.6 unkeyed hash, and a second content block holding the upstream's per-run-key notice. fathomgate forwards it in M0.
  - `test_upstream_refuses_command_injection`: LF, CRLF, `;`, `| redirect`, `| tee` and `>` each come back `isError`, and the fake device logs nothing. This is the upstream's check, not a `deny`.
...
- Status: rows 1, 2 and 22 stay `passing`, now on v1.7.1. The evidence is this PR's run until the merge run on `main`.
