...
| 2 | Dual-era handshake | 2 | netdev-ssh-mcp (go-sdk, 2026 era); upa/mcp-netmiko-server (FastMCP, 2025 era) | Both upstreams initialise; conformance suite green on the client-facing side | M0 | passing (CI on `main` at `51921ed`, 2026-09-24, both agent eras): netdev-ssh-mcp v1.6.6 initialises behind netguard at 2026-07-28; upa/mcp-netmiko-server `96e8ff3` initialises at 2025-11-25 with mcp 1.30.0, and at 2024-11-05 on its own `uv.lock` (mcp 1.6.0) after netguard restarts it once and connects with `initialize` only ([ADR 0018](../adr/0018-bound-server-discover-then-initialize-only.md)); `mcp-conformance` green. See [run notes](#run-notes) |
...
- Open follow-ups from the PR #77 and #79 reviews that do not reopen the row: T0.46 (a launcher's grandchild can outlive the restart; the CI cases start the upstream's Python directly, not through a launcher) and T0.47 (the era label after an initialize-only connect, which matters once M1 audits it).
