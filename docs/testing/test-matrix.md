...
| 2 | Dual-era handshake | 2 | netdev-ssh-mcp (go-sdk, 2026 era); upa/mcp-netmiko-server (FastMCP, 2025 era) | Both upstreams initialise; conformance suite green on the client-facing side | M0 | passing: netdev-ssh-mcp v1.7.1 initialises behind fathomgate at 2026-07-28 (tier 2 CI run 36087378002, T0.51); upa/mcp-netmiko-server `96e8ff3` (tier 2 CI on `main` at `51921ed`, 2026-09-24) initialises at 2025-11-25 with mcp 1.30.0, for a 2025-11-25 agent and a 2026-07-28 agent, and at 2024-11-05 on its own `uv.lock` (mcp 1.6.0) after fathomgate restarts it once and connects with `initialize` only ([ADR 0018](../adr/0018-bound-server-discover-then-initialize-only.md)); `mcp-conformance` green. See [run notes](#run-notes) |
