...
| 2 | Dual-era handshake | 2 | netdev-ssh-mcp (go-sdk, 2026 era); upa/mcp-netmiko-server (FastMCP, 2025 era) | Both upstreams initialise; conformance suite green on the client-facing side | M0 | passing (maintainer decision 2026-09-23: both named eras negotiate against the named real servers in CI; upa on its own 2024-11-05 lockfile hangs netguard's startup, tracked as T0.39); in tier 2 CI both upstreams initialise behind netguard: netdev-ssh-mcp at 2026-07-28 and upa/mcp-netmiko-server `96e8ff3` at 2025-11-25 (with mcp 1.30.0), with both agent eras. upa as its own `uv.lock` pins it (mcp 1.6.0) cannot start behind netguard, so the row stays open. See [run notes](#run-notes) |
