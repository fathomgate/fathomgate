...
- Environment: the upstream inherits only an allow-list from the proxy. Unix: `PATH`, `HOME`, `USER`, `LANG`, `LC_*`, `TMPDIR`. Windows (case-insensitive): `PATH`, `SystemRoot`, `SystemDrive`, `TEMP`, `TMP`, `USERPROFILE`, `APPDATA`, `LOCALAPPDATA`, `PATHEXT`, `COMSPEC`. Everything else, including `NETGUARD_*`, reaches the upstream only through `--upstream-env`, which is appended last and so overrides.
