...
Two constraints shape the flags. The tool prefix is the profile's `server` key and must not be guessed from the binary name ([profile-schema section 1](../specs/profile-schema.md#1-top-level-fields)). And an operator who passes `--policy` to a proxy that enforces nothing would believe calls are being checked when M0 forwards every one.

## Decision

We will ship `netguard serve --server <name> --upstream <path> [--upstream-env KEY=VALUE]... [-- <upstream args>...]` for M0, refuse `--policy`, `--inventory`, `--profiles` and `--audit` with exit 2 until the pipeline they configure is wired, and keep the seam for that pipeline unexported inside `internal/proxy`.

- `--server` is required and is the prefix. `--upstream` is the executable, run with no shell; its arguments follow `--`. `--upstream-env` adds to the inherited environment.
- `internal/proxy` exports `Upstream{Server, Transport}`, `Options{Version, Logger}`, `New(ctx, []Upstream, Options) (*Proxy, error)`, `(*Proxy).Run(ctx, mcp.Transport)`, `(*Proxy).Tools()`, `(*Proxy).Close()`, `Command{Path, Args, Env, Stderr}` with `Transport()`, and the constant `Name`. `New` takes a slice so multi-upstream support does not change the signature.
...
- A copied snippet that still passes `--policy` fails loudly instead of running unenforced.
