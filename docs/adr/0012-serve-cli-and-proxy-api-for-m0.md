...
| 2026-09-25 | Pointer, not a new shape: the API list gains `(*Proxy).UpstreamExited() <-chan struct{}`, which `fathomgate serve --listen` waits on to stop the listener and exit 1 when the upstream exits. It is recorded in [ADR 0016](0016-streamable-http-listener.md)'s amendments, which own the listener's surface. `serve` on stdio does not use it; its behaviour when the upstream exits is unchanged | T0.31 |
| 2026-09-25 | Pointer: the API list in *Decision* is restated, with every export added since, in [ADR 0022](0022-internal-proxy-export-surface.md) (accepted 2026-09-25), which is where the `internal/proxy` export list is now read and changed. The flags and the environment rules here are unchanged | T0.52 (L3) |
