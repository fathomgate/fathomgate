...
| Authentication | `auth.RequireBearerToken(verifier, opts)` middleware; `TokenInfo.UserID` is bound to a session at `initialize`, and a later request from another user gets 403 | The verifier (constant-time comparison over SHA-256 digests of the tokens loaded from files or the environment) and the principal name, which is the name the operator gave that token |
...
- Principals are named shared tokens, so the audit tells tokens apart, not people, and MRTR answers can never satisfy `approver_must_differ`. Accepted until per-user identity or OAuth (open question 3).
...
| 2026-09-24 | The principal text in step 4 of *Request handling*, in the go-sdk table, under *Approver identity* and in *Negative* now says what the code does: `TokenInfo.UserID` is the operator's name for that token, there are several named tokens and none unnamed, and a second principal is still not a second human | T0.40, J6 in the re-review of PR #72: open question 3 replaced the single `token:<digest>` principal of the original draft and those paragraphs were left behind |
| 2026-09-24 | `HTTPOptions` gains `OrphanTTL`, **5 minutes** by default, separate from `SessionTimeout` (30 minutes): how long a call that has ended keeps a stateful upstream's prompt from being attributed to another agent session. The same re-review widened the rule to **every** call that ends, not only a cancelled one, so the TTL now bites on ordinary traffic and an idle-session timeout is the wrong length for it. The 5 minutes is the orchestrator's recommendation, recorded here so it is easy to revisit (`defaultOrphanTTL`, profile-schema 8.5) | T0.40, J1 and J3 in the re-review of PR #72 |
| 2026-09-24 | The export surface gains a fourth top-level name, the error value `proxy.ErrMCPGODEBUG`, which `cmd/netguard` returns from its own pre-flight check instead of keeping a second copy of the same sentence. The row above, of 2026-09-23, enumerates the surface as a closed list of three; read it now as **one entry point plus its options type, its path constant and one sentinel error**. K4 asks for one refusal text in one place, and two packages cannot share an unexported one, so the fourth name is what the finding costs. The decision this record made, one exported entry point with the limits as constants rather than flags, is untouched, which is why this is an amendment and not a new record: unlike the ADR 0012 and 0008 cross-references to ADR 0018, the growth here is internal to the surface this record itself owns, so it belongs in this Amendments section with its task and its reason | T0.40, K4 in the re-review of PR #72: one refusal text in one place |
| 2026-09-24 | The orphan table behind `OrphanTTL` is bounded per principal: each call netguard cannot name to a session (every 2026-era request over the listener among them) is its own record, at most 256 keyed records per principal per upstream, and past that one overflow record per principal that refuses every session's prompts, that principal's own included. The 1024-entry global cap and its shared entry are gone. The residual, that any principal able to call a stateful upstream can keep other sessions' prompts on it refused by ending a call per `OrphanTTL` or holding one open, is accepted; token removal is the only recourse. Each refusal of a prompt is logged at Warn naming the principals behind it (`ended_calls_of`, `in_flight_of`, or the attributed call's `principal`), at most once per 10 seconds per key with a `suppressed` count. `HTTPOptions` is unchanged | T0.44 (S2 and S4 in the post-merge security review of T0.40); the maintainer's decisions of 2026-09-24 in the review of PR #82 |
| 2026-09-24 | *Sealed `requestState`* as built (`ng3.`). The envelope is bound to the **transport** as well as the principal: the agent side a call arrived on (`stdio` for the session `Proxy.Run` serves, `http` for the listener) and its principal (empty on stdio) are the envelope's AES-GCM additional authenticated data, after the prefix, and are not stored in it. A retry from another principal or over another transport therefore fails authentication, and gets the same `invalid_request_state` detail as a forged envelope (`requestState does not verify`), naming neither side; "as it does today for another tool or other arguments" holds for the reason code, not the detail. `ng2.` and `ng1.` envelopes presented after the upgrade cannot verify (the key is per process, so an upgrade has already invalidated them) and get `invalid_request_state` with `requestState was issued by an earlier netguard process; call the tool again without it`; nothing migrates, as the paragraph says. A refused `requestState` is logged at Warn with the presenting transport and principal, rate-limited like the prompt refusals. *Progress tokens*: the relay belongs to one agent request because its upstream token is fresh for that request and never mapped to two relays (a collision draws a fresh token); see the T0.48 row below. `call` carries `transport` beside `principal` for `Proxy.dispatch`. No export changes, and the principal is still attribution only | T0.30. The transport is bound because a `Proxy` can serve a local agent and the listener at once (tests do), and a principal-only binding would rest on the listener never authenticating an empty principal |
| 2026-09-24 | *Progress tokens*, corrected. As merged, T0.30's row said the relay records the session key, transport and principal of its agent request, and that was cited as a control. Those fields were written and read only by a test; they are removed. The per-request ownership rests on the upstream token alone: `watchProgress` draws it (at least 128 random bits), fresh for each agent request, redraws while it is already mapped, and the relay writes only to its request's own session under that request's context. A same-principal MRTR retry whose state verifies but whose tool or arguments changed is now logged by `warnState` too, with netguard's own reason and no argument or envelope value. No export changes | T0.48, L1 and L2 in the post-merge security review of PR #85 (Go review items 1 and 2) |
