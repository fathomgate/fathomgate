...
**2026-09-25, M1-33 (the implementing PR).** What was built, and where it differs from the text above. The first point goes beyond the record and is marked for review; the rest are factual corrections or details the record left open.

| Point | As built |
| --- | --- |
| **`internal/gate` gains `Explain` (for review)** | Section 7 said the gate gains no export. `policy eval --profile` has to print what the `Verdict` does not carry: the resolved targets with role, site and tags, `Evaluate`'s own reason and rule trace, the classifier's note (the profile class and why it changed), and the refused argument names uncapped. So `internal/gate` exports `Explain(ctx, seam.CallInfo) Explanation`, and `Decide` is now `Explain`'s verdict: both run one `decide` function, so the gate path does not fork. `TestExplainMatchesDecide` pins that `Explain(c).Verdict` equals `Decide(c)` for allow, downgrade, hold, `cannot run`, unknown, unnamed, malformed, bad target, parse errors, an unlisted tool and an annotation raise. The runner and `policy eval --profile` call `Explain`; the proxy still calls `Decide`. `Explanation` is for the operator, never the agent: its reasons name targets and counters. The proxy's export table (ADR 0022, ADR 0026) is unchanged |
| Where the loader lives | The profile-set loader moved out of `cmd/fathomgate/serve_policy.go` into a new package, `internal/configset` (`Profiles`, `EmbeddedProfiles`, `ProfileDir`, `ByServer`), together with the inventory read (`ReadInventory`: a `.csv` refused, the `configfile` checks and size cap) and decode (`ParseInventory`: strict decode and `File.Chain`). `serve`, `policy test`, and `inventory lint` and `resolve` all call it; `serve`'s messages are unchanged |
| Inventory `version` | Section 2 said an inventory is read with "`version: 1` required". The inventory decoder has no `version` key (`inventory.File` holds `devices` and `roles`), so a `version` key is a load error, for `policy test` exactly as for `serve --inventory`. inventory-schema section 3 shows `version: 1` and calls it required; that disagreement between spec and code predates this record and is reported to the inventory owner (network-safety-engineer), not changed here |
| `arguments: null` | The YAML decoder does not call the arguments decoder for `null`, so such a case has neither form and is refused with that load error ("give request.class, or request.arguments"), not as a bad mapping |
| `unnamed_args`, `malformed_args` | Compared with the decision log line's lists, which keep at most 8 names of at most 64 bytes (ADR 0026, M1-18). `[]` asserts none |
| `expect.targets` | In order, with repeats removed, as `Verdict.Targets` holds them |
| What the runner reads | The test file itself with a plain read, and its policy with `policy.Load`, as before. The `configfile` checks apply to the `--profiles` directory and files and to an inventory path (decision 9). An inline inventory is decoded by the same strict decoder |
| `policy eval --inventory` | Unchanged: read with `inventory.LoadChain`, without the `configfile` checks. Section 6 did not change it; `policy test` is the path that proves what `serve` loads |
| `policy eval` output | Adds, under `--profile`, `class_source`, `parse_error`, `unnamed_args` and `malformed_args` when set, `forwarded` and the tool error line before the trace, and a `gate` object with `--json`. Rule ids, reasons, target fields and trace notes that are not printable ASCII are quoted |
| The 45 class-given cases | Unchanged. One comment in the header of `lab-open.test.yaml` now points at `lab-open.gate.test.yaml` for the classification half of each row |
| Suites and counts | `read-only.gate.test.yaml` 75 cases, `lab-open.gate.test.yaml` 22, `prod-approval.gate.test.yaml` 15: 112 gate cases beside the 45 class-given ones. `TestRepoExamplePolicies` (moved to `internal/policytest`) fails when a policy lacks either suite or a gate suite lacks a case for row 3, 4 or 6; `TestPolicyEvalMatchesGateCases` runs `policy eval --profile` on every gate case without annotations and compares decision, rule, class and `class_source` |

**`gate.Explain`: accepted.** Accepted by the maintainer, Josh Scott, on 2026-09-25, with guard tests that internal/proxy never references Explain and that Explain matches Decide. The tests are `TestGateSurface` in `internal/proxy` (no non-test file of the package names `Explain` or `Explanation`, and `proxy.Gate`'s method set is exactly `Decide` and `Arguments`), and `TestExplainMatchesDecide` and `FuzzExplainMatchesDecide` in `internal/gate` (every outcome, including the per-call caps, the group-selector and zero-target refusals, both session caps and a call with `Counted` set, with the effect compared exactly).

**2026-09-25, M1-33 follow-up (security review of PR #197, which approved with no critical, high or medium finding).** What the follow-up PR changed; none changes a decision above.

| Point | As built |
| --- | --- |
| L1: paths reach the terminal raw | A new package, `internal/termsafe`, is the one quoting helper of the CLI and `internal/policytest` (`Quote`, `QuoteEach`, `List`), and `fail` and `inventory`'s `failTo` pass the whole error through `termsafe.Text`, which escapes C0 (except line feed and tab, which fathomgate's own multi-line hints use), DEL, C1, the line and paragraph separators and the bidirectional formatting characters. Because line feed is kept, every path that can come from a file or a directory listing is quoted where it enters a message: the test file, its `policy:` and `inventory:` paths, `configset`'s profile and inventory paths, and `internal/configfile`'s open and stat errors, which no longer repeat the raw path the operating system's error carries |
| L3: aliases and large files | `policy test` reads a test file and its policy through the `configset.MaxFile` cap (16 MiB), refuses an anchor or alias anywhere in a test file before decoding it (so nothing expands), and caps a case name at 256 bytes |
| L4: plain scalars change on the way to JSON | Section 2 said a value of another YAML type, such as a timestamp, is a load error; goccy/go-yaml kept an unquoted date as a string, read `0x1F`, `0o17`, `017`, `0b11`, `+5` and `1_000` as other numbers, and kept `1e3` and integers too large for 64 bits as strings. An unquoted value in `arguments` must now be a decimal integer as written, a decimal fraction with digits on both sides of the point, `true`, `false`, `null`, or a string that does not look like a number, a date or a time; anything else is a load error naming the line (never the value) and pointing at quoting or `arguments_json` |
| N4 and deviation 5: what `policy eval` reads | `policy eval --profile` reads the profile through `configset.ProfileFileAt` (the `configfile` checks, the size cap, the file named after its server key), and `--inventory`, in both modes, through `configset.ReadInventory` (the checks, the cap, a `.csv` refused). The *`policy eval --inventory`* row above no longer holds; nothing about it is left to accept. Still read without the `configfile` checks, and accepted: the `--policy` file of `policy eval` and the test file and policy of `policy test`, which configure a check, not an enforcement (threat model) |
| N3: absent record fields | An assertion on a decision-record field the record does not carry fails with `<field> (none: the decision record has no <field> field)`. `obligations` and `unknown_target` are on every line; for `parse_error`, `unnamed_args` and `malformed_args`, an empty list still asserts that they are absent |
...
- `internal/policytest/` (the format and the runner; before this record, `internal/policy/testfile.go`), `internal/configset/`, `internal/gate/gate.go` (`Explain`), `internal/gate/seam/seam.go`, `cmd/fathomgate/policy.go`, `cmd/fathomgate/serve_policy.go`
