...
| `Gate` interface: `Decide(ctx, seam.CallInfo) seam.Verdict`; `Arguments(server, tool string) (named []string, closed bool)` | interface | `Decide` is steps 1 to 6. `Arguments` gives the profile's named set for one tool, so the proxy can narrow the advertised `inputSchema` ([ADR 0033](0033-closed-argument-list-per-tool.md) section 4) and refuse upstream prompts during a call to a closed tool (below). `*gate.Gate` implements both | this record; `Arguments` added in M1-19 *(review)* |
...
| Argument cap | Arguments longer than 64 KiB (65,536 bytes, as the agent sent them) are denied before `Decide` with `default:bad_arguments`, class `EXEC_ARBITRARY` (unclassified), reason `the arguments are larger than 64 KiB`, and `parse_error=too_large` in the log line |
| A panic in `Decide` or the resolver | Recovered and denied: `fathomgate denied <server>.<tool>: rule default:internal_error (class EXEC_ARBITRARY): fathomgate could not decide this call, so it was not run`. An Error line names the panic's kind (a runtime error's own text, or only the panic value's Go type), never the value. `default:internal_error` is a new reserved id, produced by the proxy, never by `Evaluate` or the gate *(review)* |
| Step 5, counters | Counter key: a stateful agent session (`session:` and fathomgate's session key, over HTTP or stdio); for a stateless agent, `principal:<name>` over HTTP and `process` on stdio (decision 4). A stateful call whose session fathomgate could not name is counted with its principal or the process. `devices_touched` counts distinct target names; the proxy decides a second time when the call names a device the key has already touched, so that device is counted once (policy-schema: max_devices counts distinct devices). The read, the decision and the update of one key run under one lock, so two concurrent calls cannot both pass `max_devices` on the same count. Only forwarded calls count. A key remembers at most 4,096 names; past that every other target is counted again on each call, so the count can only run high. A stateful session's counters are dropped when it ends. `pending_holds` is 0 |
| Step 8, the log line | `p.logger`, Info, `msg=decision`, the gate's attributes; the trace is added to the same line when the logger is enabled at Debug. The proxy adds no handler of its own; `TestDecisionLine` and `TestRealGateLogInjection` pin that an argument name with a newline stays on the one line |
| Annotations | `DestructiveHint` is go-sdk's `*bool`, passed as is. go-sdk v1.8 decodes `readOnlyHint` into a plain `bool`, so the proxy reads it from the raw `tools/list` answers on the wire while `New` lists the tools (`hints.go`), keeping an absent hint nil. That works on the connections the proxy already wraps (stdio and in-memory upstreams, so every `serve` upstream). For any other upstream transport every `readOnlyHint` is nil, the raise rests on `destructiveHint`, and `New` logs a Warn when a gate is set |
| `tools/list` | With a gate, a tool whose argument list is closed (its server has a profile) is advertised with only the named properties, in `properties` and in `required`. A tool the profile does not list is advertised with no properties. The upstream's own schema is not changed and the check at call time does not change |
| Upstream prompts under a policy | With a gate, an upstream's input request (MRTR `input_required` or `elicitation/create`) during a call to a tool whose argument list is closed is refused with the profile-schema 8.2 refusal text, reason `a policy is enforced and fathomgate cannot check an answer against the server profile, so upstream prompts are not relayed`. An answer is an argument the profile never named ([ADR 0033](0033-closed-argument-list-per-tool.md) *Notes after acceptance*, N1), so it is refused like one. A server with no profile, whose arguments are not checked either, still has its prompts relayed; so does every server with no gate (M0). ADR 0014 records the same note *(review)* |
