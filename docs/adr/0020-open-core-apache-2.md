...
- **DCO, no CLA.** Outside contributions keep coming in under DCO sign-off ([CONTRIBUTING.md](../../CONTRIBUTING.md#dco-sign-off)), licensed inbound under Apache-2.0 as they go out. **Without a CLA the project holds no rights to a contribution beyond Apache-2.0, so core contributions cannot later be relicensed, for example to FSL or BSL, without the contributors' consent. The maintainer accepts this.** It is the promise that makes the core safe to depend on. The commercial repository accepts no outside contributions unless it adopts terms of its own.
...
- [CONTRIBUTING.md, DCO sign-off](../../CONTRIBUTING.md#dco-sign-off); [GOVERNANCE.md, Licence](../../GOVERNANCE.md#licence)
