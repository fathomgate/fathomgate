...
| --- | --- | --- |
| 2026-09-24 | "Only a deadline exceeded on that first attempt triggers the retry" is made exact: the bound has to run out before go-sdk has begun closing the connection. go-sdk closes the session itself inside `Connect` when it rejects an answer (an `initialize` error, an unsupported protocol version) and waits its 5-second shutdown grace, so the bound can run out during a close that an answer caused. That is an answered probe; it is not retried and its own error is reported. The code tells the two apart with a connection wrapper that records whether a close has been requested (`trackedConn`, `internal/proxy/proxy.go`), read when the bound runs out and before the attempt is cancelled. The rule is exact for the newline-delimited transports that `tracksConn` wraps (command, stdio, IO and in-memory); on an unwrapped transport (Streamable HTTP, none in M0) no close is seen, and an expired bound always counts as unanswered | S1 and Go #1 in the reviews of PR #77: the first implementation checked only that the bound had expired, restarted an upstream that had answered, and logged a false warn line. The decision (retry only an unanswered probe) is unchanged |
| 2026-09-24 | "`startupTimeout` (30 seconds) ... still bounds spawn, both attempts and `tools/list` together" held only for go-sdk's requests, not for go-sdk's own close of a failed session, which could run 5 to 10 seconds past it. netguard now kills the process of the attempt in progress when the startup context ends, and cuts the 2-second exit grace of a failed attempt to what is left of it, so the sentence holds as written, except for up to 2 seconds (the `exec.Cmd` `WaitDelay`) when a grandchild of a launcher still holds the upstream's stderr after the direct child is killed; killing the whole process tree (the S2 follow-up in the threat model) removes that | S3 and Go #2 in the reviews of PR #77: a 2-second budget returned after 7 seconds |
| 2026-09-25 | The previous row's exception, "except for up to 2 seconds (the `exec.Cmd` `WaitDelay`) when a grandchild of a launcher still holds the upstream's stderr", no longer applies to the restart, a failed attempt or the startup budget running out: the kill on each of those paths now ends the upstream's whole process tree (a process group on Unix, a Job Object on Windows), so the grandchild dies with the launcher and its stderr closes before the reap. The restart's "kills and reaps the upstream" now covers the launcher's children too. `WaitDelay` stays as the backstop for a descendant that left the group (`setsid`) and is otherwise unreachable | [ADR 0021](0021-kill-the-upstream-process-tree.md) (T0.46), which closes S2 and L1 of the reviews of PR #77 and #79. A fact change; the decision (bound, one restart, `initialize` only, the 30-second budget) is unchanged |
