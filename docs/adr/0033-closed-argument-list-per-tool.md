...
- **No profile.** Under `serve --policy` a server with no profile gets an empty one ([ADR 0027](0027-serve-policy-inventory-profiles-flags.md), note of 2026-09-25), so every tool is an unknown tool as above: any call with arguments is denied by rule `default:bad_arguments`, and `serve` warns at start. Only a caller of `internal/gate` that passes no profile at all checks nothing; `serve` never does. *(Updated 2026-09-25; this line first said nothing is checked and the fallback classifier runs, which M1-20 did not build, and [ADR 0036](0036-no-fallback-classifier.md) decides there is no fallback classifier.)*
