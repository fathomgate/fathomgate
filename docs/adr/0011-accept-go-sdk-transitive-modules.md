...
- Amended: 2026-09-23 (T0.16), 2026-09-25 (T0.46, M1-09, M1-20), facts only; the decision is unchanged. See [Amendments](#amendments).
...
| `golang.org/x/sys` | `v0.48.0` | `go.mod` direct (T0.12) | `internal/audit` → `golang.org/x/sys/windows` (audit key and log DACL on create and log resume, Windows only); `internal/secretfile` → `golang.org/x/sys/windows` (the owner and DACL check on reading the audit signing key and `--listen-token-file` files, Windows only; M1-09); `internal/configfile` → `golang.org/x/sys/windows` (the owner and write-ACE check on the `serve --policy`, `--inventory` and `--profiles` files, Windows only; M1-20); `cmd/fathomgate` → `golang.org/x/sys/windows` (Winsock error codes in `listen_windows.go`, Windows only); `internal/proxy` → `golang.org/x/sys/windows` (the upstream's Job Object, Windows only; [ADR 0021](0021-kill-the-upstream-process-tree.md)); also `segmentio/asm/cpu/x86` → `golang.org/x/sys/cpu` on every platform |
...
| 2026-09-25 | T0.46 | `golang.org/x/sys` row: the third import path, `cmd/fathomgate` → `golang.org/x/sys/windows` (`cmd/fathomgate/token_windows.go`, the `--listen-token-file` owner and DACL check, Windows only), is now listed; it has been there since T0.31 (PR #109, commit `b37a17f`). All three importers are Windows only | S3 in the Go review of PR #111: the row and CLAUDE.md named two importers of three. No module or version changed |
| 2026-09-25 | M1-09 | `golang.org/x/sys` row: the importers of `golang.org/x/sys/windows` are now `internal/audit` (key and log DACL on create and log resume), `internal/secretfile` (the owner and DACL check on reading the audit signing key and listen token files), `internal/proxy` (the upstream's Job Object) and `cmd/fathomgate` (`listen_windows.go`, Winsock error codes, since commit `fb9de35`). `cmd/fathomgate` no longer imports it for the token DACL check: `token_windows.go` is gone. All four are Windows only. No module or version changed | [ADR 0028](0028-audit-key-custody.md) moved the token file check into `internal/secretfile` so the audit key uses the same code (Go review of PR #155, item 4) |
| 2026-09-25 | M1-20 | `golang.org/x/sys` row: `internal/configfile` joins the importers of `golang.org/x/sys/windows` (`GetSecurityInfo`, `GetAce`, `LookupAccount` for the integrity check on the files `serve --policy` reads, Windows only) | PR #171 (security review M4) added the package. No module or version change |
