...
| Working name | NetGuard (placeholder; rename before first release) | Short, says what it does |
...
| --- | --- | --- | --- | --- |
| M0 Pass-through | Go proxy that spawns one stdio upstream, forwards `tools/list` and `tools/call`, prefixes tool names, speaks both protocol eras; GoReleaser binary | Official conformance suite passes on the client-facing side; Claude Code and one other client list and call tools through it | netdev-ssh-mcp | 2 weeks |
...
- [ ] Name: NetGuard is a placeholder and collides with existing products; pick something searchable before the first public commit
