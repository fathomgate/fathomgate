...
| Agent retry with a `requestState` netguard did not issue, that fails authentication, has expired, or was issued for another tool or other arguments, to another principal or over another transport (8.4), or by an earlier netguard process | `-32602`, `data: {"tool": "<name>", "reason": "invalid_request_state", "detail": "..."}`. Nothing reaches the upstream. A state issued to another principal or over another transport gets the detail a forged one gets, `requestState does not verify`, which names neither; an `ng1.` or `ng2.` state gets `requestState was issued by an earlier netguard process; call the tool again without it`. Each refusal, including a state that verifies but was issued for another tool or other arguments, is logged at Warn with the presenting transport and principal and the tool it was presented on, at most once per 10 seconds per server, transport, principal and reason (the key has no tool). The logged reason is netguard's own text: for another tool or other arguments it is `requestState was issued for another tool` or `arguments differ from the call that asked for input`, and names neither the tool the state was issued for nor any argument |
...
- Upstream, once at connect: go-sdk sends `server/discover` and, if the upstream answers with any error or names no stateless version, falls back to the initialise handshake at 2025-11-25 (and negotiates down to 2024-11-05). If that first connect has not finished within 5 seconds and go-sdk has not begun closing the connection over an answer it rejected, the probe counts as unanswered ([ADR 0018](../adr/0018-bound-server-discover-then-initialize-only.md)): netguard restarts the upstream and connects a new session on the new process with `initialize` only, at 2025-11-25 and negotiating down from there, so that upstream is stateful for the life of the process even if it would have answered the probe later (8.3). The negotiated version is logged with `upstream ready` as `protocol` and `era`.
...
*The cross-session attribution rule (T0.30).* netguard ties per-call state to the agent side in exactly these ways, and in no other:
