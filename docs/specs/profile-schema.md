...
| # | Check | Answer |
| --- | --- | --- |
| 1 | Connection cap: at most 128 open TCP connections (`proxy.LimitListener`, wired by the CLI) | Further connections wait in the kernel backlog |
...
| 8 | Stateful sessions: at most 16 open. A session-less POST to the stateful handler (which may open a session) past that | 503 with `Retry-After: 1` |
...
**Slow agents.** Every write and flush to the agent runs under a 10-second write deadline, cleared after the write so an idle stream has none. A write that cannot finish in time fails, net/http drops the connection and cancels the request, and the call's progress sender exits. A stateless call is then cancelled. A stateful call keeps running within the call caps until it ends, its session is deleted or it times out. A request body must arrive within 30 seconds: the read deadline is set only when the request has a body, and is cleared as soon as the body has been read. The `http.Server` built by `proxy.NewHTTPServer` sets `MaxHeaderBytes` 64 KiB, `ReadHeaderTimeout` 10 seconds and `IdleTimeout` 120 seconds, and no server-wide `ReadTimeout` or `WriteTimeout`, either of which would cut long SSE responses.

**Environment.** `HTTPHandler` fails if `MCPGODEBUG` is set, even to an empty value (`proxy.CheckEnvironment`, which the CLI also runs so it can exit 2).

**API.** ADR 0016 names one export, `(*Proxy).HTTPHandler(HTTPOptions) (http.Handler, error)`. `HTTPOptions` carries the tokens by principal name and the caps and timeouts above. Each zero value means the default, a negative value is an error, and no field turns a check off. The handler can be built once per proxy. The helpers the CLI needs sit next to it in `internal/proxy`: `NewHTTPServer`, `LimitListener` with `MaxConnections`, `CheckEnvironment` with `ErrMCPGODEBUG`, `TokenPrincipal`, `Principal(ctx)` and `HTTPPath`.
