...
```text
fathomgate serve --server <name> --upstream <path> [--upstream-env KEY=VALUE]... [--upstream-env-pass NAME]... [-- <upstream args>...]
```
...
Decision record: [ADR 0016](../adr/0016-streamable-http-listener.md) (accepted). This section describes `(*Proxy).HTTPHandler` in `internal/proxy` (T0.27). The `fathomgate serve --listen` flags, the token file and the lifecycle are T0.31, and T0.33 completes this section. Until then the agent side of `fathomgate serve` is stdio only (8.3).
...
| # | Check | Answer |
| --- | --- | --- |
| 1 | Connection cap: at most 128 open TCP connections (a limiting listener in `cmd/fathomgate`, wired by `--listen` in T0.31) | Further connections wait in the kernel backlog |
