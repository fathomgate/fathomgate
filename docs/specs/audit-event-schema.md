...
| `class_source` | `profile`, `fallback`, `capability_table`, `annotation_raise`, `downgrade`, `reclassify` ([classification.md](classification.md) section 2; `classify.Source`). Planned: `proxy`, for a call the proxy refused before the classifier ran (arguments over 64 KiB, a gate that failed; ADR 0026 notes, M1-19), class `EXEC_ARBITRARY`; it is in the M1 decision log line today and joins this list when M4 writes the event. |
