...
1. Look up `tool` in the profile. If found, take `class` and `class_source: profile`. If the tool has `capability_param` (a meta-tool, [profile-schema.md section 2.5](profile-schema.md#25-capability-tables)), look the argument's value up in its capability table byte for byte: a listed id gives the table's class, anything else (an unlisted id, a missing, `null` or non-string value) gives `EXEC_ARBITRARY`, always with `class_source: capability_table`, and classification ends there, since a meta-tool has no commands or config payload. If the tool has `config_params` and its class is not `EXEC_ARBITRARY`, run the config payload check (section 11); a failure sets `EXEC_ARBITRARY` with `class_source: reclassify` and ends classification, so no later step can lower it.
...
| `capability_table` | Step 1 through a capability table: every call to a meta-tool, whether the table lists its capability or not. | yes (M1-17) |
...
`internal/gate` applies this table (ADR 0026 step 3); `Classify` itself has no annotation input. The test is on the profile's class; for a meta-tool that is the class its capability table gives the call's capability (the tool's own `class` is only the `EXEC_ARBITRARY` of an unlisted one), so `readOnlyHint: false` on Meraki `execute_api` raises `getOrganizations` to `EXEC_ARBITRARY`, and the upstream's `readOnlyHint: true` leaves an unlisted capability `EXEC_ARBITRARY`. A raised call is `EXEC_ARBITRARY` with `class_source: annotation_raise`, whatever its commands say: the upstream is saying the tool's execution context is not read-only, so a raised tool is never downgraded (as if its notes carried `never-downgrade`, section 8). The raise can only make a class stricter. A call whose commands already made it `EXEC_ARBITRARY` keeps its own `class_source`, and a raise never re-runs the downgrade: re-running it once turned a raised `READ_CONFIG` tool with `show version` into `READ_OPERATIONAL` (security review of PR #162, H1). An annotation is taken as sent: the gate's input distinguishes an absent annotation from an explicit `false` or `true`, and only the explicit value counts. go-sdk v1.8 decodes an absent `readOnlyHint` as `false`, so `internal/proxy` reads each tool's `readOnlyHint` from the upstream's `tools/list` answer on the wire, before go-sdk decodes it, and keeps absence as nil (`hintCapture` in `internal/proxy/hints.go`, M1-19); `destructiveHint`, which go-sdk keeps as a pointer, is taken from go-sdk. On a transport the capture does not wrap, every `readOnlyHint` is nil and the raise rests on `destructiveHint` alone. The zero-target rule of [profile-schema section 2.2](profile-schema.md#22-targets-at-the-gate) exempts a tool by its profile class, so a raised `INVENTORY_READ` tool is decided by the rules as `EXEC_ARBITRARY`.

## 5. `EXEC_ARBITRARY` downgrade

Applied per command in `commands[]`. Every command MUST pass every check, or the class stays `EXEC_ARBITRARY`. A single failing command in a batch fails the whole call; `Result.Reason`, which `fathomgate policy eval` prints, names the first failing command by its index, never its text, and the check. The decision log line does not carry it.
...
| meraki `execute_api` | other | `capability_id: getOrganizationDevices` | EXEC_ARBITRARY | table row → INVENTORY_READ | | INVENTORY_READ | capability_table |
| meraki `execute_api` | other | `capability_id: getNetworkWirelessSsids` | EXEC_ARBITRARY | table row → READ_CONFIG | | READ_CONFIG | capability_table |
| meraki `execute_api` | other | `capability_id: rebootDevice` | EXEC_ARBITRARY | no row (a POST; the upstream indexes GET operations only) | | EXEC_ARBITRARY | capability_table |
| meraki `execute_api` | other | `capability_id: GetOrganizations` | EXEC_ARBITRARY | no row (lookup is byte for byte) | | EXEC_ARBITRARY | capability_table |
...
Tier 1 table tests in `internal/classify` cover every row in section 9. The Meraki rows are in `capability_test.go` (`TestMatrixRow18`, test-matrix row 18, through the shipped profile; `TestCapabilityClassify` with the case, padding, control-character and Unicode look-alike forms of an id; `TestMerakiCapabilityTable` against the upstream's capability set; `FuzzCapabilityLookup` and `FuzzCapabilityTableKey`), and `internal/gate` `capability_test.go` carries them from arguments to decision with annotations and raw JSON escapes. The others are in `classify_test.go` (`TestWorkedExamples`, `TestExitCriterion3` for M1 exit criterion 3 and test-matrix rows 3 and 5, the allow-prefix, blocklist and config-read tables with one positive and one negative case per entry, and the chaining and injection forms the downgrade never accepts) and `security_test.go` (the multi-line injection regressions from the security review of PR #150, with every line-break variant, through every free-form tool; the short-form config dumps of the PR #152 review; shell-quoted option injection; the 1024-byte cap; `monitor traffic` without `count`; and for section 11, the PR #158 session-escape payloads through every CLI config tool of the shipped profiles in `TestSecurityConfigSessionEscape`, every escape word and each of its abbreviations in `TestSecurityConfigEscapeWords`, ordinary configuration that must stay `WRITE_CONFIG` in `TestConfigLinesThatStayWrites`, the Junos load formats in `TestSecurityJunosLoadConfig`, the union default in `TestConfigDialectDefaultsToUnion` and JSON-string payloads in `TestSecurityConfigEscapeAsJSONString`). `internal/gate` `TestConfigSessionEscape` carries the lab-open reproduction from arguments to decision. Section 2 step 2 is covered for every tool brief 02 names on a server with no shipped profile (M1-15, M1 exit criterion 1; 137 tools on 13 servers in `internal/classify/classifytest`; both Meraki tools are in its shipped profile, M1-17). `TestFallbackBrief02` runs each through `Classify` with no profile and with the empty profile `serve` gives such a server, and `internal/gate` `TestFallbackBrief02Gate` runs each through `Decide` under the three example policies. Every row is `EXEC_ARBITRARY` with `class_source: fallback` and is never forwarded: the 112 with arguments are denied by rule `default:bad_arguments`, and the 25 with none are denied by `no-exec` with zero targets. The tools of the shipped profiles are in `TestRepoProfiles`. Each vendor allow-list and blocklist entry of sections 5.6 and 6.1 gets its cases when the vendor tables are implemented.

There is no `classify/rules.yaml`. `tools/policy-lint` validates the policy schema only; it does not classify commands, so there is no second implementation to keep in step, and the Go rules in `internal/classify/command.go` are the only copy. If a Python consumer of the command rules appears, `fathomgate` should export them from the Go source rather than load a hand-kept file.
