...
Planned, not yet implemented: reload without a restart, keeping the previous policy on validation failure ([ADR 0030](../adr/0030-reload-policy-and-inventory.md), proposed and deferred to M2; until then a policy change needs a restart of `fathomgate serve`), and a policy file hash in every audit event (M4).

## 9. Example policies shipped

| File | Purpose | Class-given cases | Gate cases |
| --- | --- | --- | --- |
| `policies/examples/read-only.yaml` | Allow the three read classes, deny everything else. The M1 announcement policy. | 12 | 77 |
