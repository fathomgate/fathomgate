...
- In M1 the decision log line lists the target in `targets` with an empty entry in `roles` and carries `unknown_target=true`. The audit event (M4) takes the same fields; its `unknown_target` flag is planned there.
...
The normaliser (`internal/gate` at the proxy, `classify.Normalize` for `fathomgate policy eval`) expands these forms before the chain runs:
