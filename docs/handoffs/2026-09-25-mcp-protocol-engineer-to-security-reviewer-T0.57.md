...
- **Not closed, recorded:** a DELETE has the same window (`cancelSession` then go-sdk's Close). I did not retire on DELETE, because go-sdk can still refuse that DELETE (a header mismatch gives 400) after fathomgate retired a live session. ~~Such a call runs until it ends or `Proxy.Close`.~~ Corrected in the re-review (N3): go-sdk's `serveStatefulDELETE` calls `Close` synchronously, and `Close` waits for the call, so the session stays registered. A DELETE does not stop fathomgate's idle clock, so `expire` fires after `SessionTimeout` and `retire` cancels the call. The bound is `SessionTimeout` (30 minutes), the same as for a call whose POST was dropped. Pinned by `TestDeleteWindowBoundedByIdleExpiry`, recorded in row 34, and followed up for M1 by T0.58.
...
3. ~~Is the DELETE window acceptable?~~ Answered in the re-review: yes, bounded by `SessionTimeout`, with T0.58 to revisit it in M1.

## Re-review round (N3, N4)

- **N3:** threat-model row 34 gives the DELETE residual and its bound (above). `TestDeleteWindowBoundedByIdleExpiry` delivers a call through the gate, drops the POST and sends DELETE. It waits for the new Debug line `agent session DELETE received`, which is logged after `cancelSession` found nothing to cancel, then opens the gate, and checks that the idle expiry (`calls_cancelled=1`) cancels the call after `SessionTimeout` and that the DELETE then answers 204. Board task T0.58 (M1 follow-up) is added.
- **Go re-review (retired-set leak):** a DELETE racing an eviction or an expiry could leave a closed session in `callLimits.retired` for good. Two changes fix it. The watcher now calls `forget` under `h.mu`, after deleting from `live`, and claims only see sessions in `live`. `expire` now retires before releasing `ls.mu`, and the watcher's `stop` waits on `ls.mu`. The lock order (`h.mu` → `limits.mu`, `ls.mu` → `limits.mu`; `limits.mu` is a leaf) is on the `callLimits` godoc. The race itself would need a new hook inside `claimEvictableLocked` or `expire` to reproduce deterministically, so I did not add one. Instead the eviction, expiry, gate and DELETE tests each wait for the retired set to empty (`waitRetiredEmpty`), and removing the `forget` call makes them fail.
- **N4:** `errSessionRetired`'s godoc points at profile-schema 8.5, and `TestSessionRetiredText` pins the exact agent-facing text the spec quotes.
