...
2. **An upstream MCP server.** This guide uses
   ...
   Do not use an older version. v1.6.6 and earlier leave some secrets in
   clear and use a hash that can be reversed
   ([GHSA-8g43-jrf3-q9vq](https://github.com/krisiasty/netdev-ssh-mcp/security/advisories/GHSA-8g43-jrf3-q9vq),
   fixed in v1.7.0). v1.7.0 and earlier let a `run_show_command`, `run_ping`
   or `run_traceroute` call add further commands for the device to run
   ([GHSA-h47r-329w-6p9h](https://github.com/krisiasty/netdev-ssh-mcp/security/advisories/GHSA-h47r-329w-6p9h),
   fixed in v1.7.1). In M0, fathomgate forwards every call without checking
   it, so the upstream's own checks are the only ones there are.
...
| `OBFUSCATION_KEY_FILE` | Path to the key file for netdev-ssh-mcp's secret tokens (see [below](#leave-netdev-ssh-mcps-obfuscation-on)) | `--upstream-env OBFUSCATION_KEY_FILE=/Users/you/.config/netdev-ssh-mcp.key` |
| `OBFUSCATION_KEY` | The same key as a value, instead of a file | `--upstream-env-pass OBFUSCATION_KEY` |
...
Give it a key of your own. Without one, netdev-ssh-mcp picks a random key
each time it starts. That is safe, but tokens from one session do not match
tokens from the next, and every result that holds a token ends with a note
from netdev-ssh-mcp asking for a key. To make a key file:
...
Then add `--upstream-env OBFUSCATION_KEY_FILE=/Users/you/.config/netdev-ssh-mcp.key`
to fathomgate's arguments, with the full path (clients do not expand `~`).
The path is not secret; the file is. Or pass the key as a value with
`--upstream-env-pass OBFUSCATION_KEY`. Use one or the other: netdev-ssh-mcp
exits at startup if both are set, or if the key is shorter than 16 bytes.

It is still not fathomgate's redaction. Treat everything the agent sees as if
it contained your secrets:
...
- Output collected with v1.6.6 or earlier may hold secrets in clear, or as
  ...
  ([GHSA-8g43-jrf3-q9vq](https://github.com/krisiasty/netdev-ssh-mcp/security/advisories/GHSA-8g43-jrf3-q9vq))
  says to treat it as sensitive and to rotate secrets whose output left your
  control.
