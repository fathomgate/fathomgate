...
| Resolver chain | The ordered inventory providers: static file, hostname patterns, upstream inventory, source of truth. Only a name authority (static file, upstream inventory, source of truth) makes a target known; hostname patterns only add attributes to a listed target (ADR 0031). |
