...
- The Go that CI and releases build with is the `toolchain` line in `go.mod` (go1.25.14); `go 1.25.0` stays the floor (ADR 0013). With the default `GOTOOLCHAIN=auto`, a local Go older than the pin downloads it on first use; a newer local Go keeps building with itself. `make toolchain-check` reports whether the Go in use is the pinned one.
- `make vulncheck` (govulncheck, pinned in the `Makefile`) must report no vulnerability reachable from our code; CI runs it on every push, every pull request and weekly. It sets `GOTOOLCHAIN` to the `go.mod` toolchain, so it scans the standard library that ships whatever Go you have installed. A fix is usually a toolchain bump (below) or a module bump, and a module bump follows ADR 0011.
- **Bumping the Go toolchain.** Dependabot does not update the `toolchain` line (dependabot/dependabot-core#13520), so a bump is manual, usually because the weekly govulncheck run failed on a new standard-library advisory or a new Go patch is out:
  1. Find the newest patch of the pinned minor: `curl -s 'https://go.dev/dl/?mode=json&include=all' | grep -o '"go1\.25\.[0-9]*"' | head -1` (`include=all` because 1.25 is no longer a supported release and the default list omits it).
  2. `go mod edit -toolchain=go1.25.N`; `go mod tidy -diff` must print nothing.
  3. `GOTOOLCHAIN=go1.25.N make toolchain-check`, `make vulncheck`, then the green gate. After `GOTOOLCHAIN=go1.25.N make build`, `go version -m bin/netguard` shows the new toolchain.
  4. Open a `build(ci): bump Go toolchain to go1.25.N` pull request with the advisory IDs it fixes; go-reviewer and security-reviewer review it. No workflow file changes. Moving the toolchain to a new Go minor also needs `GOVULNCHECK_VERSION`, the golangci-lint version and the `Dockerfile` builder checked; raising the `go` floor needs a new ADR.
