...
| Upstream prompt impersonation: an upstream elicitation shown to the human as if netguard asked | Closed in T0.3: only form elicitation crosses. The message, the form title and every property title (the property name when there is none) are prefixed `[from <server>]`; the schema is rebuilt from an allow-list and every human-visible string in it (titles, descriptions, `oneOf` titles, defaults) has control, bidi and zero-width characters escaped; enum and const values that would need escaping are refused. The prompt is refused if the message or any schema string, property name, enum or const value or default contains `[from` after case, fullwidth, Cyrillic/Greek look-alike and bracket look-alike folding, with white space and format characters removed. Server names that read as `netguard` (case, `-`, `_` and trailing digits ignored) are reserved. netguard's refusal texts never quote an upstream value. URL-mode elicitation, sampling and roots are refused; the prompt's `_meta` is dropped | Residual: the look-alike fold is not UTS #39 (mathematical alphanumerics, other scripts, combining marks and font-specific homoglyphs pass); the label on every title is the second layer |
