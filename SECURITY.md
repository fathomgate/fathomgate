...
| Upstream prompt impersonation: an upstream elicitation shown to the human as if netguard asked | Closed in T0.3: only form elicitation crosses. The message and the form title are prefixed `[from <server>]`; the schema is rebuilt from an allow-list and every human-visible string in it (property titles and descriptions, `oneOf` titles, defaults) has control, bidi and zero-width characters escaped; enum and const values that would need escaping are refused. The server name `netguard` is reserved, so no upstream can carry `[from netguard]`. URL-mode elicitation, sampling and roots are refused; the prompt's `_meta` is dropped. Property titles and descriptions are escaped, not individually prefixed | — |
...
| Prompt flood: go-sdk runs an upstream's incoming requests concurrently, so a hostile stateful upstream can fire many `elicitation/create` at once during one call | Closed in T0.3: one prompt open per call and at most 10 per call, on the stateful path as on the MRTR path; the rest are refused and the refusal is appended to the call's result | — |
...
| Forged MRTR retry: an agent reads, edits or forges a `requestState`, swaps the arguments after the human answered, or answers requests that were never asked | Closed in T0.3: netguard issues its own AES-256-GCM sealed envelope (per-process key, random nonce), with the upstream's state and the binding to server, tool, argument digest, outstanding ids and a 30-minute expiry inside the ciphertext; answers outside the allow-list are refused before the upstream sees them. Seal and open share one 96 KiB cap, so every issued state can be opened | — |
