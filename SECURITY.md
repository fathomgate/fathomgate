...
- Give upstream secrets (device passwords, `SSH_AUTH_SOCK`) with `--upstream-env-pass NAME` and set them in the client's `env` block or a keychain wrapper, never with `--upstream-env NAME=value`: arguments are visible to other local users (`ps`) and recorded by process auditing (`auditd`, Windows event 4688, Sysmon, EDR). The value then sits in the client's config file, so keep that file out of git or use the client's `${VAR}` expansion ([docs/install.md](docs/install.md#device-credentials)). Values under 4 bytes are not scrubbed from stderr, and a value an upstream returns in a tool result reaches the agent unscrubbed in M0.
