...
| Upstream prompt impersonation: an upstream elicitation shown to the human as if netguard asked | Closed in T0.3: only form elicitation crosses, prefixed `[from <server>]`, with control, bidi and zero-width characters escaped in the message and every schema string; URL-mode elicitation, sampling and roots are refused. The upstream's prompt `_meta` is dropped | — |
...
| Forged MRTR retry: an agent edits or replays a `requestState`, swaps the arguments after the human answered, or answers requests that were never asked | Closed in T0.3: netguard issues its own HMAC-signed envelope binding server, tool, argument digest, outstanding ids and a 30-minute expiry, under a per-process key; answers outside the allow-list are refused before the upstream sees them. Replay of a valid envelope within its expiry is possible; the upstream's own state decides what a repeat means | M3 binds approval envelopes to the pending id and the approver |
| A stateful upstream's `elicitation/create` attributed to the wrong call | Closed in T0.3: relayed only when exactly one call is in flight on that upstream; refused otherwise | — |
| A stateful upstream's prompt while a stateless agent is calling | Refused with a labelled tool error; the upstream call is not held open | [ADR 0014](docs/adr/0014-stateful-upstream-prompts-to-stateless-agents.md) (proposed) |
