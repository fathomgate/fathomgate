...
- `internal/proxy` dual-era (T0.3): each side's era is detected on its own (upstream once at connect, `server/discover` then the initialise fallback; agent per request) and logged with `upstream ready`. The agent's `_meta` is never forwarded, and the upstream's result `_meta` is dropped; each side sees netguard's own identity. An upstream's form elicitation reaches the agent prefixed `[from <server>]` with control characters escaped in the message and schema: as MRTR `input_required` to a 2026-07-28 agent, whose retry must carry netguard's HMAC-signed `requestState` (bound to server, tool, arguments and outstanding ids; 30-minute expiry), or as `elicitation/create` to a 2025-era agent. Answers cross only as `accept` with content, `decline` or `cancel`. Sampling, roots and URL-mode elicitation are refused; a stateful upstream's prompt to a stateless agent is refused pending ADR 0014. Invalid retries get `-32602` with `reason` `invalid_request_state` or `invalid_input_responses`. Rules in `docs/specs/profile-schema.md` section 8.4.
- Decision record `docs/adr/0014-stateful-upstream-prompts-to-stateless-agents.md` (proposed).
