...
- `internal/proxy` dual-era (T0.3): each side's era is detected on its own (upstream once at connect, `server/discover` then the initialise fallback; agent per request) and logged with `upstream ready`. The agent's `_meta` is never forwarded, and the upstream's result `_meta` is dropped, so a 2025-era agent never sees 2026-era `_meta`. An upstream's form elicitation reaches the agent prefixed `[from <server>]`, its schema rebuilt from an allow-list (at most 32 properties, 64 enum values, 16 KiB) with every human-visible string escaped and the form title labelled: as MRTR `input_required` to a 2026-07-28 agent, whose retry must carry netguard's AES-256-GCM sealed `requestState` (the upstream's state and the binding to server, tool, arguments and outstanding ids are encrypted; 30-minute expiry; 96 KiB cap measured after sealing), or as `elicitation/create` to a 2025-era agent, one prompt open per call and at most 10 per call. Answers cross only as `accept` with content, `decline` or `cancel`. Sampling, roots and URL-mode elicitation are refused; a stateful upstream's prompt to a stateless agent is refused (ADR 0014). An unattributable prompt's refusal is only appended to results, never put in place of an upstream error. Invalid retries get `-32602` with `reason` `invalid_request_state` or `invalid_input_responses`. Methods of undeclared capabilities (`prompts/*`, `resources/*`, `logging/setLevel`, `completion/complete`) get `-32601`. The server name `netguard` is reserved (`serve` exits 2). Rules in `docs/specs/profile-schema.md` section 8.4.
