...
NetGuard is a policy-enforcing MCP proxy between AI agents and network-device MCP servers. Go core (`go 1.25.0`; direct dependencies `github.com/goccy/go-yaml` and `github.com/modelcontextprotocol/go-sdk` v1.8.x, and `golang.org/x/sys` for the Windows audit key DACL), single static binary, Python companion under `tests/` and `tools/` only.
...
- The MCP `go-sdk` is pinned to one minor, currently v1.8 (it needs Go 1.25). A go-sdk bump is its own PR.
