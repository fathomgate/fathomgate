...
func TestRelabelSchema(t *testing.T) {
	...
	cases := []struct {
		...
		// A default must round-trip, so one needing escaping is dropped (R2).
		{"property strings escaped, title labelled, escaped default dropped", obj(map[string]any{"pw": map[string]any{"type": "string", "title": "t\x1b[2J", "description": "d" + rlo, "default": "x\n"}}), nil,
			`{"properties":{"pw":{"description":"d` + bs + bs + `u202e","title":"[from s] t` + bs + bs + `u001b[2J","type":"string"}},"type":"object"}`},
		...
		{"default with a backslash dropped", obj(map[string]any{"ifc": map[string]any{"type": "string", "default": "^Gi" + bs + "d+/1$"}}), nil,
			`{"properties":{"ifc":{"title":"[from s] ifc","type":"string"}},"type":"object"}`},
		{"plain default kept", obj(map[string]any{"ifc": map[string]any{"type": "string", "default": "Gi0/1"}}), nil,
