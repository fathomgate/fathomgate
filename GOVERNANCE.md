...
- Versions before `1.0.0` advance by milestone: M0 shipped `0.1.0`, M1 targets `0.2.0`, M2 targets `0.3.0`, and so on. Patch releases carry fixes only.
...
- Every release updates `CHANGELOG.md` and publishes GoReleaser binary archives, signed checksums and SBOMs. Release notes describe conformance results and known gaps. A published container image is planned; v0.1.0 provides a Dockerfile for local builds.
...
- Milestone announcements appear in GitHub Releases; the current work is tracked in [STATUS.md](STATUS.md) and the [roadmap](ROADMAP.md).
