...
Two more serve the CLI's checking tools. `internal/configset` loads the profile set (the embedded one, or a `--profiles` directory that replaces it) and an inventory file the one way `serve`, `policy test` and `inventory lint` and `resolve` load them, with the `internal/configfile` checks. `internal/policytest` reads and runs `*.test.yaml` files: a class-given case runs `Evaluate` on the class it states; a gate case runs a raw `tools/call` through `internal/gate` `Explain`, the same steps `Decide` runs for the proxy, so a policy suite proves classification and resolution as well as the rules ([ADR 0035](docs/adr/0035-policy-test-cases-run-the-gate-path.md)). `fathomgate policy eval --profile` decides through the same function.
