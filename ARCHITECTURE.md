...
| `internal/inventory` | Resolve each target to `{role, site, tags, status}` through the resolver chain: static file (or CSV import), hostname patterns, upstream inventory, then any resolver registered through the `Resolver` interface; mark stale snapshots. The live NetBox and Nautobot connectors are in the paid edition ([ADR 0034](docs/adr/0034-source-available-under-fsl.md), *Amendments*); the core ships a stub that resolves nothing | `targets[]` | `Device` per target, or `unknown` | [inventory-schema](docs/specs/inventory-schema.md) |
