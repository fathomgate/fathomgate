...
| `internal/normalize` | Canonicalise arguments using the per-server profile: `target`, `targets[]`, `commands[]`, `config_payload`. In the code this is `classify.Normalize` for commands and payload, and `internal/gate` for targets | Raw `tools/call` arguments, profile | `Request` with canonical fields | [profile-schema](docs/specs/profile-schema.md) |
