...
What runs today (M1, [ADR 0026](docs/adr/0026-m1-policy-pipeline-at-dispatch.md)): `internal/proxy` calls `internal/gate` (normalise, classify, resolve, `Evaluate`) at `Proxy.dispatch` for every call when a gate is configured, keeps the session counters, and writes one Info decision line per call. A `deny`, a `hold` and an `allow` whose `dry_run`, `diff` or `timed_rollback` cannot be met yet are all one-line tool errors naming the rule; nothing is forwarded for them. The safety, approval, redact and audit stages arrive in M2 to M4.
...
| `internal/gate` | Runs ADR 0026 steps 1 to 6 for one call: parse the arguments, take the targets exactly as the upstream receives them (hostname or IP literal, at least one where the tool reaches devices, no group selector), classify and apply annotations (raise only), resolve each target exactly, call `Evaluate` with the proxy's session counters; turns the `Decision` into forward or not, the one-line tool error (`fathomgate <denied\|cannot run\|held> <server>.<tool>: rule <id> (class <CLASS>): <reason>`) and the decision log line's fields. No I/O of its own, no state; `internal/proxy` calls it at `Proxy.dispatch` when `Options.Gate` is set (M1-19), and `serve --policy` sets it (M1-20, `cmd/fathomgate/serve_policy.go`: the policy, the inventory chain and the profiles, loaded before anything is spawned, each file read through `internal/configfile` (nobody but the owner and administrators may change it) and decoded through `internal/yamlstrict` (strict, one document, errors that never quote the file); a `--server` with no profile gets an empty one, so its calls with arguments are denied). `CallInfo` and `Verdict` live in the leaf package `internal/gate/seam`, which imports nothing from Fathomgate, so the proxy can name them without importing `classify`, `inventory` or `policy` | `seam.CallInfo` (server, tool, raw arguments, annotations, protocols and eras, principal, counters) | `seam.Verdict` (forward, decision, rule id, class, targets, tool error, log fields) | [ADR 0026](docs/adr/0026-m1-policy-pipeline-at-dispatch.md), [profile-schema 2.2](docs/specs/profile-schema.md#22-targets-at-the-gate) |
| `internal/normalize` | Canonicalise arguments using the per-server profile: `target`, `targets[]`, `commands[]`, `config_payload`. In the code this is `classify.Normalize` for commands and payload, and `internal/gate` for targets; it also reports arguments the profile does not name, for the gate to deny with `default:bad_arguments` (closed argument list, [ADR 0033](docs/adr/0033-closed-argument-list-per-tool.md); lives in `internal/classify` today) | Raw `tools/call` arguments, profile | `Request` with canonical fields | [profile-schema](docs/specs/profile-schema.md) |
| `internal/classify` | Assign one class from the profile; a tool with no profile entry is `EXEC_ARBITRARY` and never downgraded, and under `--policy` any call to it with arguments is denied by rule `default:bad_arguments` (no fallback classifier, [ADR 0036](docs/adr/0036-no-fallback-classifier.md)); downgrade `EXEC_ARBITRARY` when every command passes the allow-list; meta-tool capability tables | `Request` | `Request.class` | [classification](docs/specs/classification.md) |
| `internal/inventory` | Resolve each target to `{role, site, tags, status}` through the resolver chain: name authorities (static file or CSV import, upstream inventory, then any resolver registered through the `Resolver` interface) decide whether a target is known, and hostname patterns only enrich a target an authority lists (ADR 0031); mark stale snapshots. The live NetBox and Nautobot connectors are in the paid edition ([ADR 0034](docs/adr/0034-source-available-under-fsl.md), *Amendments*); the core ships a stub that resolves nothing | `targets[]` | `Device` per target, or `unknown` | [inventory-schema](docs/specs/inventory-schema.md) |
...
Two packages under `internal/` are helpers, not stages. `internal/secretfile` reads a secret from a file only its owner can read or replace: opened without following a final symbolic link or reparse point, then checked on the open descriptor or handle (regular file, one link, owned by the current user, mode `0600` or `0400`, or a protected owner-only DACL on Windows). Listen token files (`cmd/fathomgate`) and the audit signing key (`internal/audit`, [ADR 0028](docs/adr/0028-audit-key-custody.md)) go through it, and the redaction key will in M2. `internal/fileacl` reports whether an open file has a macOS extended ACL, which `internal/secretfile` and the audit key and log creation and resume checks (`internal/audit`) refuse because the mode bits do not show it (T0.52). It reads the ACL with `fgetattrlist(2)` through the `syscall` package, so it needs no cgo, and on every other OS it reports none.

Two more serve the CLI's checking tools. `internal/configset` loads the profile set (the embedded one, or a `--profiles` directory that replaces it) and an inventory file the one way `serve`, `policy test` and `inventory lint` and `resolve` load them, with the `internal/configfile` checks. `internal/policytest` reads and runs `*.test.yaml` files: a class-given case runs `Evaluate` on the class it states; a gate case runs a raw `tools/call` through `internal/gate` `Explain`, the same steps `Decide` runs for the proxy, so a policy suite proves classification and resolution as well as the rules ([ADR 0035](docs/adr/0035-policy-test-cases-run-the-gate-path.md)). `fathomgate policy eval --profile` decides through the same function. `internal/termsafe` is the one helper that quotes names and paths from files and escapes control and bidi characters in an error before the CLI prints it.

Data that contributors edit without Go lives outside `internal/`: `profiles/` (one YAML per upstream server), `policies/examples/` (with `*.test.yaml` cases), and `tests/fixtures/configs/` (redaction corpus). The profiles are also built into the binary: `profiles/embed.go` embeds `profiles/*.yaml` with `go:embed`, `serve --policy` uses that set unless `--profiles <dir>` replaces it, and `fathomgate version` lists it ([ADR 0027](docs/adr/0027-serve-policy-inventory-profiles-flags.md)).

Toolchain note: `go.mod` declares Go 1.26.0 as the floor ([ADR 0015](docs/adr/0015-raise-go-floor-to-1-26.md)), builds with the Go named by its `toolchain` line ([ADR 0013](docs/adr/0013-pin-go-toolchain-in-go-mod.md)) and pins go-sdk v1.8.0, which `internal/proxy` imports ([ADR 0011](docs/adr/0011-accept-go-sdk-transitive-modules.md)). With no gate (`proxy.Options.Gate` nil: `serve --no-policy`) the proxy is pass-through: `tools/list` and `tools/call` are forwarded with the `<server>.` prefix and no pipeline stage runs. With a gate, every `tools/call` runs ADR 0026 steps 1 to 8 at `Proxy.dispatch` before anything reaches the upstream, and a refused call is a one-line tool error that names its rule ([profile-schema section 8](docs/specs/profile-schema.md#8-proxy-config-m0)).

## The two MCP eras

Most network MCP servers on GitHub still run 2025-era SDKs. The current spec, 2026-07-28, removed the handshake and sessions. Fathomgate speaks both from M0.
...
| How Fathomgate holds a call | Tool error naming the pending id; agent polls `check_approval` | `input_required` whose `requestState` carries the signed pending id |
| How an upstream prompt reaches the agent (M0) | `elicitation/create` relayed by Fathomgate, prefixed `[from <server>]` | `input_required` with the relabelled request and a Fathomgate-sealed (AES-GCM) `requestState` wrapping the upstream's |
...
sequenceDiagram
  participant Agent
  participant Proxy as Fathomgate
  ...
  Approver->>Proxy: fathomgate approve <id>  (or HMAC webhook, or MRTR accept)
...
Fathomgate assumes the following. Everything it enforces follows from them.
...
| Source of truth (NetBox, Nautobot; a CSV export in the core, a live connector in the paid edition) | Trusted data, unreliable availability | A stale snapshot is used when unreachable, and every decision made from it is marked `sot: stale` in the audit event. An outage never loosens policy silently. |
...
Fathomgate is a network-semantic policy layer. It does not rebuild what generic MCP gateways already do well.
