...
4. In review (T0.4): the official conformance suite runs against the
   client-facing side for both eras (`make conformance`, CI job
   `mcp-conformance` in `.github/workflows/ci.yaml`), with expected failures
   listed per check in `tests/conformance/baseline/`.
